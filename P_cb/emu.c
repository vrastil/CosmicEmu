...
static void emu(double *xstar, double *ystar) {
    ...
    for(i=0; i<neta; i++) {
        ...
        // Convert to P(k)
        //ystaremu[i] = ystaremu[i] - 1.5*log10(mode[i % nmode_cb]);
    ...
    if(zmatch == -1) {
        for(i=0; i<nmode_cb; i++) {
            for(j=0; j<rs; j++) {
                ybyz[rs-j-1] = ystaremu[j*nmode_cb+i];
            }
        ...
        for(i=0; i<nmode_cb; i++) {
            ystar[i] = ystaremu[zmatch*nmode_cb + i];
        }
    }
...
void emu_cb(double *xstar, double *ystar, double* ext_modes){
    emu(xstar, ystar);
    for(int i = 0; i < nmode_cb; ++i) ext_modes[i] = mode[i];
}
